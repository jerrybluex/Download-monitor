...
def detect_download_file(pid: int, explicit_path: str | None) -> Path | None:
    ...
    candidates: list[str] = []
    for path in iter_open_paths(pid):
        lower = path.lower()
        if path.startswith("/") and any(token in lower for token in DOWNLOAD_FILE_TOKENS):
...
def infer_candidate_kind(command: str, file_path: Path | None) -> tuple[str, str]:
    ...
    if "brew" in lower_command or "port " in lower_command:
